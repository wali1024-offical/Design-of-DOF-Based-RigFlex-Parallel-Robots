    ...
    all_vars = [c1; c2; c3; Fx1; Fx2; Fx3; Fy1; Fy2; Fy3; Mo1; Mo2; Mo3; fai];
    ...
    %% ============ 5. 外层循环: afa 从 0 到 1, 叠加绘图 ============
    % 这里把 alpha_range 改细一点, 让绘图更好看
    alpha_range = 0:0.1:1.0;  % 每隔0.2取一个值

    % 创建 figure, 并配置坐标轴
    figure('Name','More Beautiful Plot (No Shadow)');
    hold on; grid on; box on;
    xlabel('X (m)');  % 坐标轴标签
    ylabel('Y (m)');  % 坐标轴标签
    axis square;
    set(gca, 'Position', [0.15 0.15 0.7 0.7]);  % 坐标轴居中
    xlim([-0.3 0.3]); ylim([-0.3 0.3]);         % 坐标范围
    ...
    for idx = 1:nAlpha
        ...
        % ========== 只绘制实际曲线, 不带灰色阴影 ==========
        % 保存第一根杆的绘图句柄用于图例显示
        hLegend(idx) = plot(x_def(1,:), y_def(1,:), '-', 'Color', lineColor, 'LineWidth', 2);
        ...
        % 连接“根部”（保持蓝紫色，如果想要统一改颜色也可以）
        plot([xm1_val, x_def(1,1)], [ym1_val, y_def(1,1)], '-', 'Color', [0.3 0.3 0.7], 'LineWidth', 1.5);
        plot([xm2_val, x_def(2,1)], [ym2_val, y_def(2,1)], '-', 'Color', [0.3 0.3 0.7], 'LineWidth', 1.5);
        plot([xm3_val, x_def(3,1)], [ym3_val, y_def(3,1)], '-', 'Color', [0.3 0.3 0.7], 'LineWidth', 1.5);

    end

    % 添加图例, 显示不同 alpha 值对应的颜色
    legend(hLegend, arrayfun(@(a) sprintf('\\alpha=%.1f', a), alpha_range, 'UniformOutput', false), ...
